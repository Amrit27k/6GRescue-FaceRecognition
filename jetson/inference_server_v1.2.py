...
"""Flask-based Face Recognition Inference Service for Jetson with RTSP Streaming
   (Publishes results via MQTT to an external broker, does not overlay frames)"""
import cv2
import numpy as np
import time
import os
...
class JetsonFaceRecognitionRTSP:
    def __init__(self, model_service_url=None, rtsp_port="8554", mqtt_broker_host="10.70.0.64", mqtt_port=1883):
        ...
        # MQTT Configuration
        # IMPORTANT: mqtt_broker_host should be the IP of your Edge Server (e.g., "192.168.2.101")
        self.mqtt_broker_host = mqtt_broker_host # Default to localhost if not set
    ...
    def get_frame_from_rtsp_stream(self):
        ...
        if self.rtsp_capture is None or not self.rtsp_capture.isOpened():
            ...
            self.rtsp_capture = cv2.VideoCapture(rtsp_url) 
    ...
    def detect_and_recognize_faces_yolo(self, frame):
        ...
        if self.yolo_model is None:
            try:
                ...
                logger.error(f"Failed to load YOLOv8n-face model: {e}. Please ensure 'ultralytics' is installed and the model file is accessible.")
                return [], frame.copy() 
        
        faces = []
        # Create a copy of the original frame; no overlays will be drawn on this copy.
        # This 'processed_frame' will effectively be the raw frame for the /video_feed.
        ...
        try:
            ...
            for r in results: 
                for box in r.boxes: 
                    if int(box.cls[0]) == 0: # Check if the detected object is a 'face'
                        ...
                        if w > 0 and h > 0: 
                            ...
                            if face_roi.size > 0: 
                                ...
                                recognition_result = self.recognize_face(query_features)
            ...
            logger.error(f"Error during YOLOv8n face detection/recognition: {e}")
            return [], frame.copy()
    ...
    def recognize_face(self, query_features: np.ndarray) -> Dict[str, Any]:
        """Recognize face using model service"""
        try:
            
            data = {
                "instances": [
                    {"image_feature_vector": query_features}
                ]
            }
...
def capture():
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Error publishing manual MQTT message: {e}")
...
def start_streaming():
    ...
    try:
        ...
        logger.error(f"Error starting continuous streaming: {e}")
        return jsonify({"success": False, "error": str(e)}), 500
